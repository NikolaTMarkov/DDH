...
# --- Load data -----------------------------------------------------------
 load ( '~/Library/CloudStorage/GoogleDrive-markovokean@gmail.com/My Drive/7 Iosif brain synchrony and pathways aging/Data/MD_UCSF_db_082025.RData' )

# Confirm that gender is a factor and define age groups
 firstVisit_md_long_combined <- firstVisit_md_long_combined %>%
...
select_best_model <- function ( df , alpha = 0.05 , aic_cut = -2 , w_cut = 0.70 ) {
 ...
 dat <- df %>%
		select ( MD , Age , gender ) %>%
		filter ( is.finite ( MD ) , is.finite ( Age ) , !is.na ( gender ) )
 ...
 if ( nrow ( dat ) < 10 || dplyr::n_distinct ( dat$Age ) < 5 ) {
  ...
  null_fit   <- lm (MD ~ gender ,  data = dat )
  linear_fit <- lm ( MD ~ Age + gender ,  data = dat )
  quad_fit   <- lm ( MD ~ poly ( Age , 2 ) + gender , data = dat )
...
# --- Run per tract ------------------------------------------------------------
 model_results <- firstVisit_md_long_combined %>%
			group_by ( tract ) %>%
			nest ( ) %>%
...
# Preserve tract facet order
 ...
 md_predictions <- md_predictions %>%
			mutate ( tract = factor ( tract , levels = tract_order ) )

# Plot 
	ggplot ( ) +
	geom_point ( data = firstVisit_md_long_combined ,
             aes ( x = Age , y = MD ) ,
             color = "grey70" , alpha = 0.4 , size = 0.5 ) +
  	stat_summary ( data = firstVisit_md_long_combined,
       		aes ( x = age_bin_midpoint , y = MD ) ,
       		fun = mean , geom = "point" , color = "black" , size = 1.5 ) +
	geom_line ( data = md_predictions ,
			aes ( x = age_bin_midpoint , y = predicted_MD , color = best_model ) , linewidth = 1 ) +
  	facet_wrap	(	~ tract , scales = "free_y" ) +
  	scale_color_manual ( values = c ( 	"linear" = "#d95f02" ,
                                		"non_linear" = "#1b9e77" ,
                                		"none" = "#7570b3" ) ) +
	labs ( title = "Tract MD vs Age (5-year bins)" ,
	subtitle = "Gender-adjusted Linear vs Quadratic model choice with transparent statistics",
       	x = "Age (years)" , y = "Fractional Anisotropy (MD)" , color = "Chosen model" ) +
...
# Inspect results
 ...
 # write.csv (model_results_ordered, file = paste ( path , "Statistics of MD non linear vs linear model choice.csv" , sep = "/" ) )
...
ggplot() +
  geom_point ( data = filter ( firstVisit_md_long_combined , tract %in% selected_tracts ) ,
             aes ( x = Age , y = MD ) ,
             color = "grey70" , alpha = 0.4 , size = 0.75 ) +
  stat_summary ( data = filter ( firstVisit_md_long_combined , tract %in% selected_tracts ) ,
                aes ( x = age_bin_midpoint , y = MD ) ,
               fun = mean , geom = "point", color = "black" , size = 1.5 ) +
  geom_line ( data = filter ( md_predictions , tract %in% selected_tracts ) ,
            aes ( x = age_bin_midpoint , y = predicted_MD , color = best_model ) ,
            linewidth = 1 ) +
  facet_wrap ( ~ tract , scales = "free_y" ) +
  scale_color_manual ( values = c ( "linear" = "#d95f02" ,
                                "non_linear" = "#1b9e77" ,
                                "none" = "#7570b3" ) ) +
  labs ( title = "Selected Pathways: MD vs Age (Sex-adjusted)" ,
       x = "Age" ,
       y = "Fractional Anisotropy (MD)" ,
       color = "Model Match" ) +
  theme_bw ( base_size = 13 ) +
