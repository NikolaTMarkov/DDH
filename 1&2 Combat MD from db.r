#:NTM 2025 apr analysis DTI Data
 ...
 library ( rstatix ) #For wilcoxon test of MD ~ CDR
...
load ( "/Users/nikolamarkov/Library/CloudStorage/GoogleDrive-markovokean@gmail.com/My Drive/7 Iosif brain synchrony and pathways aging/Data/UCSF_db_Branch_Summer2024.RData" )
 ...
 dti_md <- as_tibble ( UCSF_db$imaging_dti_combat_md )
...
#Associate with age and select only the first recorded visit
 ...
 firstVisit_md_long <- firstVisit_md %>%
...
firstVisit_md_long_combined <- firstVisit_md_long %>%
					 mutate(
...
outlier_detection_robust <- firstVisit_md_long_combined %>%
  group_by(tract , gender) %>%
  arrange(Age , .by_group = TRUE ) %>%
  mutate(
    # Loess fit per tract
    loess_fit = predict(loess(MD ~ Age, span = 0.75 , data = pick( everything () ) ) ),
    
    # Residuals
    residuals = MD - loess_fit,
...
# Plot for males
p_male <- ggplot(outlier_male, aes(x = Age, y = MD)) +
...
# Plot for females
p_female <- ggplot(outlier_female, aes(x = Age, y = MD)) +
  geom_point(aes(color = is_outlier), size = 2, alpha = 0.7) +
  geom_line(aes(y = loess_fit), color = "blue", linewidth = 1) +
  facet_wrap(~tract, scales = "free_y") +
  scale_color_manual(values = c("chartreuse4", "red")) +
...
#outilier in one tract removes the whole scan 
 OutliersMD <- outlier_detection_robust %>% filter ( is_outlier == TRUE ) %>% 
 											distinct( UnQID) %>%  
 											pull( UnQID )
 firstVisit_md_long_combined <- firstVisit_md_long_combined %>% 
 								filter ( !UnQID %in% OutliersMD ) 
 
 # SOM Figure 1 plot of the md against age
 
# Plot: LOESS by gender and CDR class
 firstVisit_md_long_combined %>%
	group_split ( gender ) %>%
	
	walk( ~ {
		df_gender  <-  .x  %>%  arrange ( CDR_class )
		g <- unique ( df_gender$gender )
		p <- ggplot ( df_gender , aes ( x = Age , y = MD , color = CDR_class ) ) +
...
#Resistent linear model to identify if AGE affects MD
...
# Bootstrap function to extract Age coefficient
 boot_age_coef <- function ( data , indices ) {
	fit <- rlm ( MD ~ Age + gender , data = data [ indices , ] )
	coef ( fit ) [ "Age" ] }

# Run rlm and bootstrap CI per tract
 boot_results <- firstVisit_md_long_combined %>%
	group_split ( tract ) %>%
		map_df ( function ( df ) {
    tract_name <- unique ( df$tract )

    	# Fit RLM
    	model <- rlm ( MD ~ Age + gender , data = df )
    	est <- coef ( model ) [ "Age" ]
 ...
 ggplot ( boot_results , aes ( x = fct_reorder ( tract , estimate ) , y = estimate ) ) +
	...
	coord_flip ( ) +
	labs (
    title = "Gender adjusted RLM effect of Age on MD" ,
...
within_age_stats <- firstVisit_md_long_combined %>%
  group_by(age_group, gender) %>%
  filter(n_distinct(CDR_class) >= 2) %>%
  rstatix::wilcox_test(MD ~ CDR_class, p.adjust.method = "fdr") %>%
...
#PLOT firstVisit_md_long_combined %>%
 firstVisit_md_long_combined %>%
	group_split ( gender ) %>%
		walk ( ~ {
		df_gender <- .x
		...
		p <- ggplot ( df_gender , aes ( x = age_group , y = MD , fill = CDR_class ) ) +
      	geom_jitter (
        aes ( color = CDR_class ) ,
        alpha = 0.4 , size = 0.5,
		...
		scale_fill_manual ( values = color_mapping ) +
      	scale_color_manual ( values = color_mapping ) +
      	labs (
        title = paste ( "MD by Age Group, Tract, and CDR —" , g ) ,
        x = "Age Group" ,
...
#save ( list = c ( "color_mapping" , "firstVisit_md_long_combined" , "mta" , "UCSF_db" ) , file = "MD_UCSF_db_082025.RData" )                      
